...
func TestGetBearerToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.authHeader != "" {
				headers["Authorization"] = []string{tt.authHeader}
			}
			httpHeaders := http.Header(headers)

			gotToken, err := GetBearerToken(httpHeaders)
			if (err != nil) != tt.expectingError {
				t.Errorf("GetBearerToken() error = %v, expectingError %v", err, tt.expectingError)
				return
			}
